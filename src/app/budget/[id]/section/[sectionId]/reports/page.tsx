    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          <button
            ...
            className="mt-1 flex size-8 shrink-0 items-center justify-center rounded-lg text-foreground transition-colors hover:bg-muted border border-border"
          >
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="border border-border rounded-lg bg-card p-4 text-center">
          ...
          <p className={cn("text-xl font-semibold tabular-nums", textColor)}>
      ...
      <div className="space-y-2">
        <div className="flex justify-between items-center">
          ...
          <span className={cn("text-sm font-semibold tabular-nums", textColor)}>{percentage}%</span>
        </div>
      ...
      <div className="space-y-4">
        ...
        {displayCats.length === 0 ? (
          <div className="border border-border rounded-lg bg-card flex flex-col items-center justify-center py-12 text-center">
            <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-lg border border-border bg-muted">
              <Plus className="h-6 w-6 text-muted-foreground" strokeWidth={ICON_STROKE} />
            </div>
          ...
          displayCats.map((item, idx) => {
            if (item.type === "own") {
              ...
              return (
                <div key={item.id} className={cn("border border-border rounded-lg bg-card transition-opacity", dragId === item.id && "opacity-50")} draggable={displayCats.length > 1} onDragStart={(e) => handleDragStart(e, item.id)} onDragOver={(e) => handleDragOver(e, item.id)} onDrop={(e) => handleDrop(e)} onDragEnd={handleDragEnd}>
                  <div className="p-5 sm:p-7">
                    ...
                    <div className="sm:hidden">
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        {displayCats.length > 1 && (
                          <div className="shrink-0 cursor-grab active:cursor-grabbing touch-none text-muted-foreground/40 hover:text-foreground transition-colors" aria-label="Drag to reorder">
                            <GripVertical className="size-5" strokeWidth={ICON_STROKE} />
                          </div>
                        )}
                      </div>
                      <div className="mb-4 pb-4 border-b border-border space-y-2">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <span className={cn("text-sm font-medium tabular-nums", catTextColor)}>
                            {catPct}% {tDash("used")}
                          </span>
                        </div>
                      </div>
                      <div className="flex gap-2 mb-4">
                        <Button variant="outline" size="sm" onClick={() => router.push(`/${budgetBase}/${params.id}/section/${params.sectionId}/category/${cat.category.id}`)} className="flex-1 gap-1.5">
                          <BarChart3 className="size-3.5" strokeWidth={ICON_STROKE} />
                          {tActions("reports")}
                        </Button>
                        <Button variant="outline" size="sm" onClick={() => setEditingCategory(cat.category)} className="flex-1 gap-1.5">
                    ...
                    <div className="hidden sm:flex items-center justify-between min-h-[44px] mb-4">
                      ...
                      <div className="flex items-center gap-2 mr-6">
                        ...
                        <Button variant="outline" size="sm" onClick={() => setEditingCategory(cat.category)} className="gap-1.5">
                          <Settings className="size-3.5" strokeWidth={ICON_STROKE} />
                          {tActions("adjust")}
                        </Button>
                      </div>
                      <div className="text-right">
                        ...
                        <p className={cn("text-sm font-medium tabular-nums mt-1", catTextColor)}>
                    ...
                    <div className="mt-3">
                      <div className="h-2.5 w-full overflow-hidden rounded-full bg-muted">
                        <div
                          className={cn("h-full rounded-full transition-all duration-300", catProgressColor)}
            ...
            return (
              <div key={item.id} className={cn("border border-border/50 border-dashed rounded-lg bg-card transition-opacity", dragId === item.id && "opacity-50")} draggable={displayCats.length > 1} onDragStart={(e) => handleDragStart(e, item.id)} onDragOver={(e) => handleDragOver(e, item.id)} onDrop={(e) => handleDrop(e)} onDragEnd={handleDragEnd}>
                <div className="p-5 sm:p-7">
                  {/* Linked badge */}
                  ...
                  <div className="sm:hidden">
                    ...
                    <div className="mb-4 pb-4 border-b border-border space-y-2">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className={cn("text-sm font-medium tabular-nums", lcTextColor)}>
                          {lcPct}% {tDash("used")}
                        </span>
                      </div>
                    </div>
                    <div className="flex gap-2 mb-4">
                      <Button variant="outline" size="sm" onClick={() => router.push(`/${budgetBase}/${params.id}/section/${params.sectionId}/category/${lcat.category.id}`)} className="flex-1 gap-1.5">
                        <BarChart3 className="size-3.5" strokeWidth={ICON_STROKE} />
                        {tActions("reports")}
                      </Button>
                      <Button variant="outline" size="sm" onClick={() => setEditingLinkedCat({ cat: lcat, link: lc.link, sourceName: lc.sourceBudgetName })} className="flex-1 gap-1.5">
                        <Settings className="size-3.5" strokeWidth={ICON_STROKE} />
                        {tActions("adjust")}
                      </Button>
                    </div>
                  ...
                  <div className="hidden sm:flex items-center justify-between min-h-[44px] mb-4">
                    ...
                    <div className="flex items-center gap-2 mr-6">
                      <Button variant="outline" size="sm" onClick={() => router.push(`/${budgetBase}/${params.id}/section/${params.sectionId}/category/${lcat.category.id}`)} className="gap-1.5">
                        <BarChart3 className="size-3.5" strokeWidth={ICON_STROKE} />
                        {tActions("reports")}
                      </Button>
                      <Button variant="outline" size="sm" onClick={() => setEditingLinkedCat({ cat: lcat, link: lc.link, sourceName: lc.sourceBudgetName })} className="gap-1.5">
                        <Settings className="size-3.5" strokeWidth={ICON_STROKE} />
                        {tActions("adjust")}
                      </Button>
                    </div>
                    <div className="text-right">
                      ...
                      <p className={cn("text-sm font-medium tabular-nums mt-1", lcTextColor)}>
                  ...
                  <div className="mt-3">
                    <div className="h-2.5 w-full overflow-hidden rounded-full bg-muted">
                      <div
                        className={cn("h-full rounded-full transition-all duration-300", lcProgressColor)}
